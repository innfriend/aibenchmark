...
export const OOMWarningGuard: React.FC<OOMWarningGuardProps> = ({
  ...
  const estimatedModelWeightsGb = (model.parameterCountM * 1e6 * bytesPerParam) / 1e9;
  ...
  const availableVramGb = selectedHardware.memoryGb;
  const isOOM = totalRequiredVramGb > availableVramGb;
  const memoryUtilizationPct = Math.min(100, Math.round((totalRequiredVramGb / availableVramGb) * 100));
    ...
    return (
      <div className="bg-rose-950/40 border border-rose-800/80 rounded-2xl p-4 space-y-2 text-xs font-mono text-rose-300 animate-fadeIn">
        <div className="flex items-center gap-2 font-bold text-rose-400">
          ...
          <span>Out-Of-Memory (OOM) Risk Detected on {selectedHardware.name}</span>
        </div>
        <p className="text-slate-300 font-sans text-xs leading-relaxed">
          {model.name} in <strong>{precision}</strong> requires approximately <strong>{totalRequiredVramGb} GB</strong> VRAM, but {selectedHardware.name} only has <strong>{availableVramGb} GB</strong>.
        </p>
