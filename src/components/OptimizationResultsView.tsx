...
export const OptimizationResultsView: React.FC<OptimizationResultsViewProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'pareto' | 'table' | 'flamegraph' | 'batch' | 'tco' | 'export'>('pareto');
  ...
  const bestLatencyResult = [...results].sort((a, b) => a.latencyMs - b.latencyMs)[0] || results[0];
  const bestEfficiencyResult = [...results].sort((a, b) => (b.throughputFps / b.powerConsumptionWatts) - (a.throughputFps / a.powerConsumptionWatts))[0] || results[0];
  ...
  const currentSweep = job.batchSweepData[selectedSweepHw] || job.batchSweepData[defaultHwId] || [];
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#07090E] text-slate-100 overflow-y-auto p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <div className="bg-[#0D1322] border border-[#1E293B] rounded-3xl p-6 sm:p-8 space-y-4">
        ...
        {/* Compiler Insight Summary Pill */}
        {job.aiInsights && (
          <div className="bg-[#07090E] border border-cyan-900/40 rounded-2xl p-4 flex items-start gap-3 text-xs">
        ...
        {/* Tab Navigation */}
        <div className="flex items-center gap-2 pt-2 overflow-x-auto border-t border-[#1E293B]">
          {[
            ...
            { id: 'export', label: 'Code & Container Export', icon: Code2 },
          ].map((tab) => {
            const Icon = tab.icon;
                ...
                <div className="p-4 bg-[#07090E] border border-cyan-800/40 rounded-2xl space-y-1 text-xs">
                  <div className="flex items-center justify-between font-mono">
                    ...
                    <span className="text-white">
                      {(bestEfficiencyResult.throughputFps / bestEfficiencyResult.powerConsumptionWatts).toFixed(2)} FPS/W
                    </span>
                  </div>
                  <div className="font-mono text-slate-300">{bestEfficiencyResult.hardwareName}</div>
                  <p className="text-[11px] text-slate-500 font-sans">Consumes only {bestEfficiencyResult.powerConsumptionWatts}W active power.</p>
                </div>
              </div>
            </div>
