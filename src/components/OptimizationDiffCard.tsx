...
export const OptimizationDiffCard: React.FC<OptimizationDiffCardProps> = ({
  ...
  const vramSavingsPct = Math.round(((baseline.vramGb - optimized.vramGb) / baseline.vramGb) * 100);
  const speedupFactor = (baseline.latencyMs / optimized.latencyMs).toFixed(1);
  const latencyReductionPct = Math.round(((baseline.latencyMs - optimized.latencyMs) / baseline.latencyMs) * 100);
  const throughputGainPct = Math.round(((optimized.throughputFps - baseline.throughputFps) / baseline.throughputFps) * 100);
  const costSavingsPct = Math.round(((baseline.costPerMillion - optimized.costPerMillion) / baseline.costPerMillion) * 100);

  const defaultVerdict = `By applying AWQ 4-bit quantization and SwiGLU kernel fusion, ${modelName} memory footprint drops by ${vramSavingsPct}%. The model now comfortably fits on a single ${optimized.hardwareName} with a ${speedupFactor}x latency reduction, slashing monthly cloud GPU costs by ${costSavingsPct}%.`;
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-[#07090E] border border-[#1E293B] rounded-2xl p-4 space-y-2">
          ...
          <div className="flex items-baseline justify-between pt-1 font-mono">
            <div className="text-xs text-slate-500 line-through">{baseline.vramGb} GB ({baseline.precision})</div>
            <ArrowRight className="w-3 h-3 text-slate-600" />
            <div className="text-base font-bold text-emerald-400">{optimized.vramGb} GB ({optimized.precision})</div>
          </div>
        ...
        <div className="bg-[#07090E] border border-[#1E293B] rounded-2xl p-4 space-y-2">
          ...
          <div className="flex items-baseline justify-between pt-1 font-mono">
            <div className="text-xs text-slate-500 line-through">{baseline.latencyMs} ms</div>
            <ArrowRight className="w-3 h-3 text-slate-600" />
            <div className="text-base font-bold text-amber-300">{optimized.latencyMs} ms</div>
          </div>
          <div className="text-[10px] text-slate-400 font-sans">
        ...
        <div className="bg-[#07090E] border border-[#1E293B] rounded-2xl p-4 space-y-2">
          ...
          <div className="flex items-baseline justify-between pt-1 font-mono">
            <div className="text-xs text-slate-500 line-through">{baseline.throughputFps} FPS</div>
            <ArrowRight className="w-3 h-3 text-slate-600" />
            <div className="text-base font-bold text-cyan-300">{optimized.throughputFps} FPS</div>
          </div>
          <div className="text-[10px] text-slate-400 font-sans">
        ...
        <div className="bg-[#07090E] border border-[#1E293B] rounded-2xl p-4 space-y-2">
          ...
          <div className="flex items-baseline justify-between pt-1 font-mono">
            <div className="text-xs text-slate-500 line-through">${baseline.costPerMillion.toFixed(2)}</div>
            <ArrowRight className="w-3 h-3 text-slate-600" />
            <div className="text-base font-bold text-emerald-400">${optimized.costPerMillion.toFixed(2)}</div>
          </div>
          <div className="text-[10px] text-slate-400 font-sans">
