...
export const HardwareSandboxView: React.FC<HardwareSandboxViewProps> = ({ onNavigate }) => {
  ...
  const rankedRecommendations = HARDWARE_CATALOG.map(hw => {
    ...
    if (metrics.monthlyCost > constraints.maxMonthlyBudgetUsd) {
      fitScore -= 40;
      warnings.push(`Exceeds budget ($${metrics.monthlyCost.toFixed(0)}/mo vs $${constraints.maxMonthlyBudgetUsd}/mo limit)`);
    } else {
      reasons.push(`Under budget ($${metrics.monthlyCost.toFixed(0)}/mo)`);
    }

    if (metrics.throughputFps < constraints.minThroughput) {
      fitScore -= 30;
      warnings.push(`Throughput below SLA (${metrics.throughputFps.toFixed(0)} vs ${constraints.minThroughput} required)`);
    } else {
      reasons.push(`Meets throughput SLA (${metrics.throughputFps.toFixed(0)} FPS / Tok/s)`);
    }

    if (metrics.latencyMs > constraints.maxLatencyMs) {
      fitScore -= 25;
      warnings.push(`Latency higher than SLA (${metrics.latencyMs.toFixed(1)}ms vs ${constraints.maxLatencyMs}ms)`);
    } else {
      reasons.push(`Sub-SLA latency (${metrics.latencyMs.toFixed(1)}ms)`);
    }
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-[#07090E] p-4 rounded-2xl border border-[#1E293B] space-y-3 font-mono text-xs">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-bold text-white text-sm">{metrics.latencyMs.toFixed(2)} ms</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-400">Throughput:</span>
                        <span className="font-bold text-cyan-300 text-sm">{metrics.throughputFps.toFixed(0)} {metrics.tokensPerSec ? 'Tok/s' : 'FPS'}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-400">Efficiency:</span>
                        <span className="font-bold text-emerald-300">{metrics.efficiency.toFixed(2)} FPS/Watt</span>
                      </div>
                      <div className="flex justify-between items-center">
                    ...
                    <div className="pt-3 border-t border-[#1E293B] space-y-1 text-xs font-mono">
                      <div className="flex justify-between">
                        ...
                        <span className="text-white font-bold">${hw.hourlyCloudCostUsd?.toFixed(2) || '0.00'}/hr</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-400">Monthly 24/7 Cost:</span>
                        <span className="text-emerald-400 font-bold">${metrics.monthlyCost.toFixed(0)}/mo</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-400">Cost / 1M Inferences:</span>
                        <span className="text-cyan-300 font-bold">${metrics.costPerMillion.toFixed(2)}</span>
                      </div>
                    </div>
                  </div>
                ...
                >
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 my-4 text-xs font-mono">
                    <div className="bg-[#07090E] p-3 rounded-xl border border-[#1E293B]">
                      ...
                      <span className="font-bold text-white text-sm">{rec.metrics.latencyMs.toFixed(1)} ms</span>
                    </div>
                    <div className="bg-[#07090E] p-3 rounded-xl border border-[#1E293B]">
                      <span className="text-slate-400 block text-[10px]">Throughput:</span>
                      <span className="font-bold text-cyan-300 text-sm">{rec.metrics.throughputFps.toFixed(0)} FPS</span>
                    </div>
                    <div className="bg-[#07090E] p-3 rounded-xl border border-[#1E293B]">
                      <span className="text-slate-400 block text-[10px]">Monthly TCO:</span>
                      <span className="font-bold text-emerald-300 text-sm">${rec.metrics.monthlyCost.toFixed(0)}/mo</span>
                    </div>
                    <div className="bg-[#07090E] p-3 rounded-xl border border-[#1E293B]">
